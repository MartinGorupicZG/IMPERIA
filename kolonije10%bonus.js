...
function init() {
	...
	values.innerHTML = `<span>TERAIN TYPE</span><span>BONUS TYPE</span><span>BONUS SIZE</span><span>DISTANCE TO EMPIRE</span><span>LOCATION</span>`;
	...
	function showData(col) {
		...
		for (var blockId = 0; blockId < blocks.length; blockId++) {
			...
			for (var i = 0; i < block.length; i++) {
				...
				if ((colony.length = 4)) {
					//showC( tooltip.vl, tooltip1.vl,tooltip2.vl,tooltip3.vl,tooltip4.vl,tooltip5.vl,tooltip6.vl, x, y);

					if (block[i].obs[0].ttp[1] !== undefined) {
						if (tooltip2.vl == Top) {
							showC(tooltip.vl, tooltip1.vl, tooltip2.vl, tooltip3.vl, x, y);
						}
					}
				}
			}
	...
	function showC(a, b, c, d, e, f) {
		let player = document.createElement('div');

		player.innerHTML = `<span>${a}</span><span>${b}</span><span>${c}</span><span>${d}</span><span>X:${e}  Y:${f}</span>`;
		player.style.width = '100%';
		player.style.display = 'flex';
		player.style.alignItems = 'center';
		player.style.justifyContent = 'space-evenly';
		player.style.borderBottom = '1px solid black';
		player.style.minHeigth = '1.5rem';
