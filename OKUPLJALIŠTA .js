...
// @namespace  Carlinjo
// @version    0.1
// @description Traženje okupljališta
...
values.innerHTML = `<span>TIP TERENA</span><span>SAVEZ</span><span>UDALJENOST</span><span>LOKACIJA</span>`;
...
function showData(col) {
	...
	for (var blockId = 0; blockId < blocks.length; blockId++) {
		...
		for (var i = 0; i < block.length; i++) {
			...
			if (colony.length == 3) {
				showC(tooltip.vl, tooltip1.vl, tooltip2.vl, x, y);
			}
		}
	}
}
function showC(a, b, c, d, e) {
	let player = document.createElement('div');

	player.innerHTML = `<span>${a}</span><span>${b}</span><span>${c}</span><span>X:${d}  Y:${e}</span>`;
